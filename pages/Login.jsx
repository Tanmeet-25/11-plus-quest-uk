...
export default function Login() {
  ...
  const APP_URL = "https://11-quest-uk.base44.app";
        ...
        <div style={{ background: "white", borderRadius: 24, padding: "28px 24px", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
          ...
          <form onSubmit={handle}>
            {mode === "signup" && (
              <div style={{ marginBottom: 14 }}>
                ...
                <input
                  ...
                  required
                  style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "2px solid #e5e7eb", fontSize: 15, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}
                />
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ width: "100%", padding: "14px", borderRadius: 14, border: "none", background: loading ? "#e5e7eb" : "linear-gradient(135deg,#4F46E5,#7C3AED)", color: loading ? "#9ca3af" : "white", fontWeight: 800, fontSize: 16, cursor: loading ? "not-allowed" : "pointer", boxShadow: !loading ? "0 4px 20px rgba(79,70,229,0.4)" : "none" }}
            >
          ...
          <p style={{ textAlign: "center", fontSize: 12, color: "#9ca3af", marginTop: 16, marginBottom: 0 }}>
            ...
            >
              {mode === "login" ? "Register free" : "Sign in instead"}
            </span>
          </p>
        </div>

        <p style={{ textAlign: "center", color: "rgba(255,255,255,0.4)", fontSize: 11, marginTop: 16 }}>
          Progress saves automatically once you're signed in 🔒
        </p>
      </div>
    </div>
  );
